...
# Каталог логов (переживает перезапуски на Streamlit Cloud в рамках инстанса)
...
def get_logger(name: str = "app", level: str | None = None) -> logging.Logger:
    ...
    # Плюс вывод в stdout (видно в «Logs» на Streamlit)
    stream = logging.StreamHandler()
    stream.setFormatter(logging.Formatter("%(asctime)s | %(levelname)s | %(name)s | %(message)s"))
    logger.addHandler(stream)
    logger.propagate = False
    logger.info(f"Logger initialized: level={logger.level}, file={LOG_FILE}")
    return logger
